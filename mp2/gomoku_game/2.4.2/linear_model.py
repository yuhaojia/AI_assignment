...
class LinearModel(object):
    ...
    def forward(self, x):
        ...
        self.x = x
        c = np.ones(self.x.shape[0])
        self.x = np.column_stack((self.x, c))
        f1 = np.dot(self.x, self.w)
        f = np.expand_dims(f1, axis=1)
        print(f.shape)
        """
        N = x.shape[0]
        ndims = x.shape[1]
        x_part = np.ones((N, ndims + 1))
        x_part[:,:-1] = x
        self.x = x_part
        f = np.dot(x_part, self.w)
