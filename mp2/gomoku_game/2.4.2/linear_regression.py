...
class LinearRegression(LinearModel):
    ...
    def backward(self, f, y):
        ...
        diff_fy = f - y      
        total_grad = np.dot(self.x.T, diff_fy) + self.w_decay_factor*self.w
    ...
    def total_loss(self, f, y):
        ...
        """
        diff1_fy = f - y
        total_loss = 0.5*np.linalg.norm(diff1_fy, ord=2)**2 + 0.5*self.w_decay_factor*np.linalg.norm(self.w, ord=2)**2
