...
class qlearning(object):
    ...
    def updateQTable(self, state, action, reward, nextstate, terminal):
        state_index = self.getStateIndex(state)
        nextstate_index = self.getStateIndex(nextstate)
        self.nsatable[state_index][action+1] += 1
        if not terminal:
            q_target = reward + GAMMA * np.amax(self.qtable[nextstate_index])
        else:
            q_target = reward
        self.qtable[state_index][action+1] += float(ALPHA) / float(ALPHA + self.nsatable[
            state_index][action+1]) * (q_target - self.qtable[state_index][action+1])
    ...
    def writeFile(self):
        np.savetxt('qtable.txt', self.qtable, delimiter=',')
